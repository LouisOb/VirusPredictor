...
#Tup=['GTTG','CAAG','AATT','ACAA','CAAC','TTTA','GTTA','TCAT','TTAA','AAAA','AAAG','AGTT','TGTG']
...
virus_comb = np.concatenate((MERS_ORFs,SARS_1_ORFs,SARS_2_ORFs),axis=1)#,HEPA_ORFs,HERPES_ORFs),axis=1)
...
pk.dump(pca.components_,open("3tuple_pca.pk","wb"))

'''
for i in range(len(Tup)):
		plt.hist([SARS_2_ORFs[i][j][0] for j in range(len(SARS_2_ORFs[i]))],bins=20,alpha=.5,facecolor="green",label="SARS-CoV 2")
		plt.hist([SARS_1_ORFs[i][j][0] for j in range(len(SARS_1_ORFs[i]))],bins=20,alpha=.5,facecolor="red",label="SARS")
		plt.hist([MERS_ORFs[i][j][0] for j in range(len(MERS_ORFs[i]))],bins=20,alpha=.5,facecolor="blue",label="MERS")
...
plt.scatter(virus_comb[0],virus_comb[1],c=colour)
